    ...
    def alphabeta(move, p=player):
        try:
            ...
            if p==player:
                ...
                for o in outcomes:
                	min_element = min(o,min_element)
                if min_element <= alpha:
                    return min_element
                beta = min(beta,min_element)
            else:
            	max_element = 1
                for o in outcomes:
                	max_element = max(o,max_element)
                if max_element >= beta:
                    return max_element
                alpha = max(alpha,max_element)
        finally:
        	board.undoMove(move)

    #moves = [(move, evaluateMove(move)) for move in board.getValidMoves()]
    moves = [(move, alphabeta(move)) for move in board.getValidMoves()]
